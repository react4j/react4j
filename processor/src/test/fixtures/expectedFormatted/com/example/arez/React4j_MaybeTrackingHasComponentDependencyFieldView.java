...
abstract class React4j_MaybeTrackingHasComponentDependencyFieldView
    extends MaybeTrackingHasComponentDependencyFieldView {
  @Nonnull
  ...
  private static ViewConstructorFunction getConstructorFunction() {
    ...
    if (React.enableViewNames()) {
      Js.asPropertyMap(viewConstructor)
          .set("displayName", "MaybeTrackingHasComponentDependencyField");
    }
  ...
  private static void $$react4j$$_validateInputValues(@Nonnull final JsPropertyMap<Object> inputs) {
    ...
    if (React.shouldCheckInvariants()) {
      Guards.apiInvariant(
          () -> null != raw$model,
          () ->
              "Required input named 'model' is missing from view named 'MaybeTrackingHasComponentDependencyFieldView' so it was either incorrectly omitted or a null value has been incorrectly specified.");
    }
  }
  ...
  private static final class LiteNativeView extends react4j.internal.NativeView
    ...
    LiteNativeView(@Nullable final JsPropertyMap<Object> inputs) {
      super(inputs);
      final MaybeTrackingHasComponentDependencyFieldView.Model model =
          inputs.getAsAny(Inputs.model).cast();
      view = Disposable.isDisposed(inputs.getAsAny(Inputs.model).cast())
          ? null
          : new Arez_React4j_MaybeTrackingHasComponentDependencyFieldView(this, model);
      if (React.shouldValidateInputValues()) {
        assert null != inputs;
        React4j_MaybeTrackingHasComponentDependencyFieldView.$$react4j$$_validateInputValues(
            inputs);
      }
    }
