...
final class NonTrackingHasAutoObserveFieldBuilder {
  ...
  private static class Builder implements Step1 {
    ...
    private final ReactElement _element =
        ReactElement.createViewElement(React4j_NonTrackingHasAutoObserveFieldView.Factory.TYPE);

    @Override
    @Nonnull
    @Contract(pure = true)
    public final ReactNode model(@Nullable final NonTrackingHasAutoObserveFieldView.Model model) {
      _element.setKey((model instanceof Keyed
              ? Keyed.getKey(model)
              : model instanceof Identifiable
                  ? Identifiable.<Object>getArezId(model)
                  : String.valueOf(model))
          + (React.enableViewNames()
              ? "_NonTrackingHasAutoObserveFieldView_35f9d1be"
              : NonTrackingHasAutoObserveFieldView.class.getName()));
