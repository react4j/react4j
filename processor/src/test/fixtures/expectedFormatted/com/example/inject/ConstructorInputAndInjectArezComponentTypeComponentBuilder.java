...
final class ConstructorInputAndInjectArezComponentTypeComponentBuilder {
  ...
  private static class Builder implements Step1 {
    ...
    private final ReactElement _element = ReactElement.createViewElement(
        React4j_ConstructorInputAndInjectArezComponentTypeComponent.Factory.TYPE);
    ...
    public final ReactNode model(
        @Nonnull final ConstructorInputAndInjectArezComponentTypeComponent.Model model) {
      _element.setKey(Identifiable.<Object>getArezId(model)
          + (React.enableViewNames()
              ? "_ConstructorInputAndInjectArezComponentTypeComponent_8e180ee8"
              : ConstructorInputAndInjectArezComponentTypeComponent.class.getName()));
      assert Disposable.isNotDisposed(model);
      Objects.requireNonNull(model);
      _element.input(
          React4j_ConstructorInputAndInjectArezComponentTypeComponent.Inputs.model, model);
      return build();
    }
