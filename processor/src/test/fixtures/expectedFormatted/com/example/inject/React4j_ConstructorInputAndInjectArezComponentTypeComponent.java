...
abstract class React4j_ConstructorInputAndInjectArezComponentTypeComponent
    extends ConstructorInputAndInjectArezComponentTypeComponent {
  @Nonnull
  ...
  private static ViewConstructorFunction getConstructorFunction() {
    ...
    if (React.enableViewNames()) {
      Js.asPropertyMap(viewConstructor)
          .set("displayName", "ConstructorInputAndInjectArezComponentTypeComponent");
    }
  ...
  private static void $$react4j$$_validateInputValues(@Nonnull final JsPropertyMap<Object> inputs) {
    ...
    if (React.shouldCheckInvariants()) {
      Guards.apiInvariant(
          () -> null != raw$model,
          () ->
              "Required input named 'model' is missing from view named 'ConstructorInputAndInjectArezComponentTypeComponent' so it was either incorrectly omitted or a null value has been incorrectly specified.");
    }
  }
  ...
  private static final class NativeView extends react4j.internal.NativeView
    ...
    NativeView(@Nullable final JsPropertyMap<Object> inputs) {
      ...
      view = Disposable.isDisposed(inputs.getAsAny(Inputs.model).cast())
          ? null
          : React4j_ConstructorInputAndInjectArezComponentTypeComponentFactory.create(this);
      if (React.shouldValidateInputValues()) {
        assert null != inputs;
        React4j_ConstructorInputAndInjectArezComponentTypeComponent.$$react4j$$_validateInputValues(
            inputs);
      }
    }
