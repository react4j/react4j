...
abstract class React4j_ImmutableMixedInputModel extends ImmutableMixedInputModel {
  ...
  String getMyMutableProp() {
    if (React.shouldCheckInvariants()) {
      return null != $$react4j$$_nativeView.inputs().getAsAny(Inputs.myMutableProp)
          ? $$react4j$$_nativeView.inputs().getAsAny(Inputs.myMutableProp).asString()
          : null;
    } else {
  ...
  private static final class NativeView extends react4j.internal.NativeView
    ...
    NativeView(@Nullable final JsPropertyMap<Object> inputs) {
      ...
      if (React.shouldCheckInvariants()) {
        myImmutableProp = null != inputs.getAsAny(Inputs.myImmutableProp)
            ? inputs.getAsAny(Inputs.myImmutableProp).asString()
            : null;
      } else {
