...
abstract class React4j_ImmutablePropTypes extends ImmutablePropTypes {
  ...
  React4j_ImmutablePropTypes(
      @Nonnull final react4j.internal.NativeView $$react4j$$_nativeView,
      @Nullable final ImmutablePropTypes.MyComponent myProp,
      @Nullable final String myOtherProp,
      final int stillAnotherProp,
      @Nullable final ImmutablePropTypes.KeyedComponent bobsProp,
      @Nullable final ImmutablePropTypes.Foo someProp,
  ...
  static final class Inputs {
    static final String stillAnotherProp =
        React.shouldMinimizeInputKeys() ? "a" : "stillAnotherProp";
  ...
  private static final class NativeView extends react4j.internal.NativeView
    ...
    NativeView(@Nullable final JsPropertyMap<Object> inputs) {
      ...
      if (React.shouldCheckInvariants()) {
        myProp = null != inputs.getAsAny(Inputs.myProp)
            ? inputs.getAsAny(Inputs.myProp).cast()
            : null;
      } else {
      ...
      String myOtherProp;
      if (React.shouldCheckInvariants()) {
        myOtherProp = null != inputs.getAsAny(Inputs.myOtherProp)
            ? inputs.getAsAny(Inputs.myOtherProp).asString()
            : null;
      } else {
      ...
      if (React.shouldCheckInvariants()) {
        bobsProp = null != inputs.getAsAny(Inputs.bobsProp)
            ? inputs.getAsAny(Inputs.bobsProp).cast()
            : null;
      } else {
      ...
      if (React.shouldCheckInvariants()) {
        someProp = null != inputs.getAsAny(Inputs.someProp)
            ? inputs.getAsAny(Inputs.someProp).cast()
            : null;
      } else {
      ...
      Object object;
      if (React.shouldCheckInvariants()) {
        object = null != inputs.getAsAny(Inputs.object)
            ? inputs.getAsAny(Inputs.object).cast()
            : null;
      } else {
        object = Js.uncheckedCast(inputs.getAsAny(Inputs.object));
      }
      view = Disposable.isDisposed(Js.uncheckedCast(inputs.getAsAny(Inputs.myProp)))
          ? null
          : new Arez_React4j_ImmutablePropTypes(
              this, myProp, myOtherProp, stillAnotherProp, bobsProp, someProp, object);
    }
