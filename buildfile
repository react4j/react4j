...
define 'react4j' do
  ...
  define 'downstream-test' do
    ...
    test.compile.enhance do
      ...
      Java::Commands.java 'react4j.downstream.CollectHeartRateMonitorBuildStats', { :classpath => cp, :properties => properties } unless ENV['BUILD_STATS'] == 'no'
  ...
  ipr.add_testng_configuration('processor',
                               :module => 'processor',
                               :jvm_args => "-ea #{FORMATTER_JDK_EXPORTS.join(' ')} -Dreact4j.output_fixture_data=true -Dreact4j.fixture_dir=src/test/fixtures")
