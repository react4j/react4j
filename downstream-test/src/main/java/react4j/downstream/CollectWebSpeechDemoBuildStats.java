...
public final class CollectWebSpeechDemoBuildStats
{
  ...
  {
    Gir.go( () -> {
      WorkspaceUtil.forEachBranch( "react4j-webspeechdemo",
                                   "https://github.com/react4j/react4j-webspeechdemo.git",
                                   Collections.singletonList( "master" ),
                                   context -> buildBranch( context, WorkspaceUtil.getVersion() ) );
      WorkspaceUtil.collectStatistics( Collections.singletonList( "webspeechdemo" ), build -> true, false );
    } );
  }
                                   ...
                                   @Nonnull final String version )
  {
    ...
    final boolean initialBuildSuccess = WorkspaceUtil.runBeforeBuild( context, () -> {
      final Path archiveDir = WorkspaceUtil.getArchiveDir( context.workingDirectory, "webspeechdemo.before" );
    ...
    WorkspaceUtil.runAfterBuild( context, initialBuildSuccess, () -> {
      ...
      final Path archiveDir = WorkspaceUtil.getArchiveDir( context.workingDirectory, "webspeechdemo.after" );
      buildAndRecordStatistics( context.appDirectory, archiveDir );
    }, () -> {
      final Path dir = WorkspaceUtil.getArchiveDir( context.workingDirectory, "webspeechdemo.after" );
  ...
  private static void buildAndRecordStatistics( @Nonnull final Path appDirectory, @Nonnull final Path archiveDir )
  {
    ...
    // Perform the build
    Ruby.buildr( "clean", "package", "EXCLUDE_GWT_DEV_MODULE=true", "GWT=react4j-webspeechdemo" );
  ...
  private static void archiveStatistics( @Nonnull final Path archiveDir )
  {
    ...
    properties.setProperty( "webspeechdemo.size", String.valueOf( getJsSize( archiveDir ) ) );
    properties.setProperty( "webspeechdemo.gz.size", String.valueOf( getJsGzSize( archiveDir ) ) );
  ...
  private static void archiveBuildrOutput( @Nonnull final Path archiveDir )
  {
    ...
    WorkspaceUtil.archiveDirectory( currentDirectory
                                      .resolve( "target/gwt_compile_reports/react4j.webspeechdemo.WebSpeechDemoProd" ),
  ...
  private static long getJsSize( @Nonnull final Path archiveDir )
  {
    return WorkspaceUtil.getFileSize( archiveDir.resolve( "assets" )
                                        .resolve( "webspeechdemo" )
                                        .resolve( "webspeechdemo.nocache.js" ) );
  }
  ...
  private static long getJsGzSize( @Nonnull final Path archiveDir )
  {
    return WorkspaceUtil.getFileSize( archiveDir.resolve( "assets" )
                                        .resolve( "webspeechdemo" )
                                        .resolve( "webspeechdemo.nocache.js.gz" ) );
  }
}
